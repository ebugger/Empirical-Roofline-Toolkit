...
public class LogarithmicAxis extends ValueAxis<Number> {
	...
	public class IllegalLogarithmicRangeException extends Exception {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 1898736125911675197L;
	...
	private void bindLogBoundsToDefaultBounds() {
		logLowerBound.bind(new DoubleBinding() {
			...
			protected double computeValue() {
				return log2(lowerBoundProperty().get());
			}
		});
		logUpperBound.bind(new DoubleBinding() {
			...
			@Override
			protected double computeValue() {
				return log2(upperBoundProperty().get());
			}
		});
	}
	...
	protected List<Number> calculateTickValues(double length, Object range) {
		...
		if (range != null) {
			...
			//double logLowerBound = log2(lowerBound.doubleValue());
			double logUpperBound = log2(upperBound.doubleValue());
	
			for (double i = -4; i <= logUpperBound; i += 1) {
	...
	public double getDisplayPosition(Number value) {
		double delta = logUpperBound.get() - logLowerBound.get();
		double deltaV = log2(value.doubleValue()) - logLowerBound.get();
		if (getSide().isVertical()) {
